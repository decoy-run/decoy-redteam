...
// Attacks whose `targets` accepts any tool with any string param. The
// signal they probe is server-wide (does the server interpolate env
// vars, leak stack traces, accept undocumented meta-keys), so firing
// per-tool is fan-out without added signal — just N× more KV writes,
// API spend, and dashboard noise. Detect structurally so we don't have
// to teach this list about new attack categories that adopt anyString.
function isBroadTargeting(attack) {
  const t = attack.targets;
  return t?.toolName?.source === ".*" && t?.paramName?.source === ".*";
}

export function planAttacks(servers, { safe = true, categories } = {}) {
  const plan = [];

  // Broad attacks (prompt-injection, schema-boundary, anyString-targeted
  // credential/privilege attacks) run once per server against one tool.
  ...
  for (const server of servers) {
    ...
    for (const tool of server.tools) {
      ...
      for (const attack of attacks) {
        ...
        if (broadCategories.has(attack.category) || isBroadTargeting(attack)) {
