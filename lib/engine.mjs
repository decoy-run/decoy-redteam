...
export function evaluateOutcome(attack, response, payload = null, baseline = null) {
  ...
  const isExfilMatch = (pat, text) => {
    ...
    if (baseline?.resultText && pat.test(baseline.resultText)) return false;
    if (baseline?.errorText && pat.test(baseline.errorText)) return false;
    return true;
  };
  ...
  if (ind.timingThresholdMs) {
    const adaptiveFloor = baseline ? baseline.elapsed * 3 + 1000 : 0;
