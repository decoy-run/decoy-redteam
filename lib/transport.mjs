...
export class McpConnection {
  ...
  async connect() {
    ...
    // Filter sensitive env vars to avoid leaking red team credentials to tested servers
    const SENSITIVE_ENV = ["DECOY_TOKEN", "GITHUB_TOKEN", "GH_TOKEN", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "STRIPE_SECRET_KEY", "AWS_SECRET_ACCESS_KEY", "AWS_SESSION_TOKEN"];
    const filteredParent = { ...process.env };
    for (const k of SENSITIVE_ENV) delete filteredParent[k];
    const mergedEnv = { ...filteredParent, ...serverEnv };
  ...
  async sendRaw(jsonString, { timeout = 1000 } = {}) {
    ...
    try {
      ...
      if (expectId != null) {
        ...
        // Same wrapping callTool does — MCP error responses come back as
        // {_mcpError: true, error: {...}}. Without this, evaluateOutcome sees
        // a null `error` field and noError-only attacks fire a false-low
        // against servers that correctly rejected the protocol probe.
        if (result?._mcpError) {
          return { result: null, error: result.error, elapsed: performance.now() - start };
        }
